...
func getClockFromTime(t time.Time) time.Time {
	...
	return time.Time{}.Add(clock)
}

func (j *Job) fitsExecutionTimeInterval(t time.Time) bool {
	actualTime := getClockFromTime(t)
	return actualTime.After(j.ExecutionTimeFrom.GetTime()) && actualTime.Before(j.ExecutionTimeTo.GetTime())
}
...
func (j *Job) Run() {
	...
	for {
		...
		if j.fitsExecutionTimeInterval(time.Now()) {
